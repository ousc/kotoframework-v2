...
val result: List<Map<String, Any>> = 
			Join(User(), ShoppingCart(), Good()){ user, cart, good ->
				leftJoin(cart).on(user.id == cart.id && user.age > 35)
				rightJoin(cart).on(user.id == cart.id && user.age > 35)
			}
User().select { user ->
